...
LIVE_SOURCES = [
    f"https://www.youtube.com/{CHANNEL_HANDLE}/streams?sort=dd",
    f"https://www.youtube.com/{CHANNEL_HANDLE}/live",
    f"https://www.youtube.com/{CHANNEL_HANDLE}/videos?view=2&live_view=502&sort=dd",
]
...
YOUTUBE_URL_PATTERN = re.compile(
    r"https?://(?:www\.)?(?:youtube\.com/live/|youtu\.be/|youtube\.com/watch\?v=)[A-Za-z0-9_-]{6,}"
)
...
# ----------- Discovery -----------
def _extract_flat(url: str):
    with YoutubeDL(YDL_OPTS_LIST) as ydl:
        return ydl.extract_info(url, download=False) or {}

def list_recent_live_entries():
    # 1) Try each channel tab variant
    for src in LIVE_SOURCES:
        try:
            info = _extract_flat(src)
    ...
    # 2) Fallback: ytsearchdate (most-recent first) for likely MR Live uploads/streams
    # We’ll over-fetch and filter
    query = 'ytsearchdate40:"Majority Report w/ Sam Seder MR Live"'
    try:
        info = _extract_flat(query)
        entries = info.get("entries") or []
        return entries
    except (DownloadError, ExtractorError):
        return []
...
def pick_target_stream(entries, target_date):
    # Normalize into comparable candidates
    cands = []
    for e in entries:
        title = (e.get("title") or "").lower()
        # Filter obvious non-shows
        if any(s in title for s in ["clip", "members", "premiere", "after show", "highlight"]):
            continue
        duration = e.get("duration") or 0
        # Convert upload_date (YYYYMMDD UTC) to ET date if present
...
def find_funhalf_url_for_video(video_url):
    ...
    # 1) Description
    url = extract_funhalf_from_text(info.get("description") or "")
    if url:
        return url
    # 2) Pinned comment
    ...
    if pinned:
        ...
        if url:
            return url
    # 3) Any top comments mentioning Fun Half
...
def main():
    ...
    if not entries:
        return 0  # still publish placeholder
    ...
    # Early run (before 12:25 ET) — allow retry job to append later
    ...
    if not url:
        ...
        if prev_chosen:
            ...
            if url:
                append_item_to_feed(OUTPUT_PATH, f"Fun Half – {prev.date()} (from previous weekday’s live)", url, now)
        return 0

    append_item_to_feed(OUTPUT_PATH, f"Fun Half – {target.date()}", url, now)
    return 0
